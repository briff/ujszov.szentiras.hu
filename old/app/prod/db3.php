...
<script type="text/javascript">
  ...
  (function() {
    var u=(("https:" == document.location.protocol) ? "https" : "http") + "://sutda.itk.ppke.hu/piwik/";
...
//		print ("<font face='Palatino Linotype' size='5'>");
		...
		if ($egy_sor=mysql_fetch_row($eredmeny)) {
			for ($c = 0; $c < count($egy_sor); $c++) {
				$egy_sor[$c] = stripslashes($egy_sor[$c]);
			}
			print ("<tr>\n");
			for ($v=0;$v<=5;$v++) {
				if ($v==4) {
...
#	Konkordancia szerinti elso
	...
	$hepax=false;
	if ($feh != $fh) {
		print("<a href=\"javascript:ugras($feh)\">Első azonos sz&oacute;t&aacute;ri alak&uacute; sz&oacute;:</a> ");
		print($elozo[0]."\n");
	} elseif ($feh != $fkh) {
		print("Ez az első ilyen sz&oacute;alak\n");
	} else {
		$hepax=true;
...
#	Konkordancia szerinti elozo/kovetkezo
	...
	$kovetkezo=fhbollink($fkh);
	if ($feh) {
		print("<a href=\"javascript:ugras($feh)\">Előző azonos sz&oacute;t&aacute;ri alak&uacute; sz&oacute;:</a> ");
		print($elozo[0]."<br>\n");
	}
	if ($fkh) {
		print("<a href=\"javascript:ugras($fkh)\">K&ouml;vetkező azonos sz&oacute;t&aacute;ri alak&uacute; sz&oacute;:</a> ");
...
#	$eredmeny=mysql_query("SELECT fh FROM $ab WHERE gk=(SELECT gk FROM $ab WHERE
	...
	$elozo=fhbollink($feh);
	$kovetkezo=fhbollink($fkh);
	print("<a href=\"javascript:ugras($feh)\">Abc-rendben előző sz&oacute;alak:</a>");
	print($elozo[0]."<br>\n");
	print("<a href=\"javascript:ugras($fkh)\">Abc-rendben k&ouml;vetkező sz&oacute;alak:</a>");
	print($kovetkezo[0]."<br>\n");
